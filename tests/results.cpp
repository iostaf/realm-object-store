...
TEST_CASE("notifications: async delivery") {
    ...
    auto make_remote_change = [&] (int new_value=5){
        auto r2 = coordinator->get_realm();
        r2->begin_transaction();
        r2->read_group().get_table("class_object")->set_int(0, 0, new_value);
    ...
    SECTION("remote changes made before adding a callback from within a callback are not reported") {
        auto results2 = results;
        NotificationToken token2, token3;
        token2 = results2.add_notification_callback([&](CollectionChangeSet, std::exception_ptr) {
            token2 = {};
        });
        advance_and_notify(*r);
        ...
        token2 = results.add_notification_callback([&](CollectionChangeSet, std::exception_ptr) {
            ...
            make_remote_change();
            coordinator->on_change();
            token3 = results2.add_notification_callback([&](CollectionChangeSet c, std::exception_ptr) {
                called = true;
                REQUIRE(c.empty());
